...
def compute_contribution(df: pl.LazyFrame, columns: list[str]):
    """Compute each player's percentage contribution to the team per-fixture."""
    ...
    # Compute player contribution ratios
    df = df.with_columns(
        [
            (pl.col(column) / pl.col(f"team_{column}"))
            .fill_nan(0)
            .alias(f"{column}_contribution")
