...
def compute_rolling_mean(
    ...
    # Create expressions for computing rolling means
    rolling_mean_expressions = []
    resulting_columns = []

    for c, w in zip(columns, windows, strict=True):
        alias = f"{c}_rolling_mean_{w}{suffix}"
        rolling_mean_expressions.append(
            pl.col(c).rolling_mean(window_size=w, min_periods=1).over(over).alias(alias)
        )
        resulting_columns.append(alias)

    # Compute all rolling means in one go
    subset = subset.with_columns(rolling_mean_expressions)
